...
template Test(msg_bytes) {
	...
	component and[133][num_bytes];
	component multi_or[2][num_bytes];
	signal states[num_bytes+1][134];
	...
	for (var i = 0; i < num_bytes; i++) {
		state_changed[i] = MultiOR(133);
		eq[0][i] = IsEqual();
		eq[0][i].in[0] <== in[i];
		eq[0][i].in[1] <== 98;
		and[0][i] = AND();
		and[0][i].a <== states[i][133];
		...
		multi_or[0][i].in[8] <== eq[4][i].out;
		multi_or[0][i].in[9] <== eq[5][i].out;
		...
		eq[24][i].in[1] <== 41;
		and[108][i] = AND();
		and[108][i].a <== states[i][107];
		and[108][i].b <== eq[24][i].out;
	...
	for (var i = 0; i <= num_bytes; i++) {
		final_state_result.in[i] <== states[i][130];
	}
	...
	for (var i = 0; i < msg_bytes; i++) {
		is_consecutive[msg_bytes-1-i][0] <== states[num_bytes-i][130] * (1 - is_consecutive[msg_bytes-i][1]) + is_consecutive[msg_bytes-i][1];
