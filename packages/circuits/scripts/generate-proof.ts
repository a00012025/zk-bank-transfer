...
program.option("--silent", "No console logs");
...
async function generate() {
  const inputPath = path.join(OUTPUT_DIR, "input.json");
  if (!fs.existsSync(inputPath)) {
    throw new Error("Could not find input file at " + inputPath);
  }
  const circuitInputs = JSON.parse(fs.readFileSync(inputPath).toString());
  ...
  const vkey = JSON.parse(
    fs
      .readFileSync(path.join(BUILD_DIR, `${CIRCUIT_NAME}.vkey.json`))
      .toString()
  );
